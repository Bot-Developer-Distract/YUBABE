...
module.exports = {
  ...
  aliases: ['h', 'catch', 'batthu', `huntthu`],
  usage: "Yhunt",
  run: async (client, message, args) => {
    //if (message.author.id !== `896739787392819240`) return message.channel.send(`Bot đang update, bạn đợi chút nha`)
    ...
    if (!huntthu) await db.set(`${message.author.id}.nvhuntthu`,0)
    if(inday.after && huntthu < 3) {
     huntthu += 1
    } else if (inday.after && huntthu >=3) {
     huntthu = 0 
    } else {
      await client.timeout(`${message.author.id}missionhuntthu`)
    }
    ...
    if (a + b < limit) {
      ...
      let lang = await checklang(message, false, false, false)
      if (lang == `vi`) messagess = messagess[0]
      if (lang == `en`) messagess = messagess[1]
      await message.channel.send({ content: messagess, files: [attachment] })
      ...
      collector.on('end', async collected => {
        if (collected.size > 0) {
          ...
          if (lang == `vi`) messagess = messagess[0]
          if (lang == `en`) messagess = messagess[1]
          return message.channel.send(messagess)
        }
...
with screenshot for unbanning!`
          ...
          if (lang == `vi`) messagess = messagess[0]
          if (lang == `en`) messagess = messagess[1]
          return message.author.send(messagess)
        }
      });
    }
    else {
      var zoopoints2 = await zoopointSchema.findOne({ zooid: message.author.id })
      if (!zoopoints2) {
        const addzp = new zoopointSchema({ zooid: message.author.id, quanlity: 0 })
        await addzp.save()
      }
      const zoopoints = await zoopointSchema.findOne({ zooid: message.author.id })
      const author = message.author.id
      let cash = await client.cash(author)
      let errorCash = [
        `${client.emo.fail} | **${message.author.username}**, bạn còn không có nỗi 15 đồng Ycoin hunt thú??`,
        `${client.emo.fail} | **${message.author.username}**, you don't even have 15 Ycoin to hunt... just try Daily or Work!`
      ]
      if (cash < 15) return await checklang(message, message, false, errorCash)
      await client.tru(author, 15)
      let heso = 1
      let x2 = false
      let lucky = false
      ...
      let huntmsg = [
        `**${message.author.username}**, ${buffmsg} || Bạn hunt được :
${thusanduoc}
\`Điểm zoo : +${point}\``,
        `**${message.author.username}**, ${buffmsg} || You've hunted :
${thusanduoc}
\`Zoo points : +${point}\``
      ]
      await checklang(message, message, false, huntmsg)
...
async function checklang(message, msg, dms, array) {
  const { QuickDB } = require("quick.db")
  const db = new QuickDB()
  let lang = ''
  const language = await db.get(`${message.guild.id}.language`)
  if (!language || language == 'vi') lang = "vi"
  else if (language == 'en') lang = "en"
  else lang = language
  if (!array) return lang
  let messagess
  if (lang == 'vi') {
    messagess = array[0]
  }
  ...
  if (msg) {
    await message.channel.send(messagess)
  }
  else if (dms) {
    await dms.send(messagess)
  }
  else if (!msg && !dms) return lang
}
